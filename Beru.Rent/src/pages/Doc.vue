  ...
  <div id="app" ref="document">
    ...
    <div id="element-to-convert">
        ...
        <p style="padding-top: 30px;text-align: center;">
          ДОГОВОР ПРОКАТА №{{ docNumber }}
<br>
г. {{ docTown }}                            		                   	                 {{ todayDate }}г.
<br>
</p>

<p style="padding-left: 70px; padding-right: 30px; text-align: left;">
ФИО  {{ ownerFio }}, ИИН {{ ownerIin }}, именуемый в дальнейшем «Арендодатель», с одной стороны, и ФИО {{ tenantFio }}, ИИН {{ tenantIin }}, именуемый (-ая) в дальнейшем «Арендатор», с другой стороны, далее совместно именуемые «Стороны»,  заключили настоящий договор о нижеследующем:
<br>
...
Наименование: {{ itemTitle }}<br>
Комплектность: {{ itemDesc }}<br>
Особые примечания: {{ extraConditions }}<br>
...
1.3. Оценочная стоимость имущества, предоставленного по настоящему Договору проката, составляет {{ dealCost }}Тенге.<br>
...
- начало – {{ dealDateBegin }}г.; окончание – {{ dealDateEnd }}г.<br>
...
<br>3.3. Передача имущества в прокат и возврат имущества Арендатором Арендодателю осуществляются по адресу: г. {{ dealAddress }} начиная с времени аренды.
...
4.1. За пользование имуществом, предоставленным в прокат по настоящему договору, Арендатор уплачивает Арендодателю арендную плату в размере {{ dealCost }} Тенге.<br>
4.2. Арендатор уплачивает Арендодателю депозит в размере {{ dealDepositCost }}Тенге.<br>
...
export default {
  ...
  methods: {
    fetchDocumentData() {
      ...
      axios.get(getUri)
        .then(response => {
          this.todayDate = response.data.todayDate;
          this.docNumber = response.data.docNumber;
          this.docTown = response.data.docTown;
          this.itemTitle = response.data.itemTitle;
          this.itemDesc = response.data.itemDesc;
          this.extraConditions = response.data.extraConditions;
          this.dealDepositCost = response.data.dealDepositCost;
          this.dealDateBegin = response.data.dealDateBegin;
          this.dealDateEnd = response.data.dealDateEnd;
          this.dealAddress = response.data.dealAddress;
          this.ownerFio = response.data.ownerFio;
          this.ownerIin = response.data.ownerIin;
          this.ownerPhone = response.data.ownerPhone;
          this.ownerEmail = response.data.ownerEmail;
          this.tenantFio = response.data.tenantFio;
          this.tenantIin = response.data.tenantIin;
          this.tenantPhone = response.data.tenantPhone;
          this.tenantEmail = response.data.tenantEmail;
        })
        .catch(error => {
